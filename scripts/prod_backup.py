...
backup_lines = check_output(backup_cmd, shell=True).decode("utf-8").split("\n")
backup_name = None
for line in backup_lines:
    if "backup" in line:
        # there are weird escape sequences in backup_name
        backup_name = "".join([c for c in line if c.isalnum() or c in {".", "_"}])
assert backup_name is not None
print(backup_name)

copy_cmd = f"docker cp {postgres_id}:/backups/{backup_name} backups"
print(copy_cmd)
result = check_output(copy_cmd, shell=True)
print(result)

ssh_cmd = f"scp homepage:site/backups/{backup_name} backups"
print(ssh_cmd)
