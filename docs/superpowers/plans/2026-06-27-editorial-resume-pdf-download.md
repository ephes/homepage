...
- `homepage/static/resume/*.pdf` — **commit** the two generated PDFs (NOT gitignored). They ride with the repo and deploy as static assets, so Chromium runs only locally during `render-pdfs`, never on the server or the visitor's machine. (Task 5)
- `justfile` — **modify**: `render-pdfs` recipe that self-starts a throwaway server, renders, and stops it (one command, no manual `:8003`). (Task 5)
...
## Task 5: Download links + self-starting render recipe + commit the PDFs
...
- Modify: `justfile`
- Commit: the two generated `homepage/static/resume/*.pdf` (NOT gitignored — they deploy as static assets so Chromium stays local-only)
...
- [ ] **Step 6: Add the self-starting justfile recipe**

The PDFs are committed artifacts (no `.gitignore` — they deploy as static assets; Chromium therefore runs only here, locally). Make `render-pdfs` a one-command recipe: it starts a throwaway runserver on a free port (using the dev DB, which holds the seeded résumé), waits until it answers, renders, then always stops the server.
...
- [ ] **Step 7: Re-render via the recipe and commit the PDFs**

Run: `cd /Users/katha/gitprojects/homepage && just render-pdfs`
Expected: two `wrote …` lines; both PDFs present in `homepage/static/resume/`.
...
git add homepage/templates/django_resume/pages/editorial/resume_cv.html \
        ...
        homepage/static/cover-letter/cover.css justfile \
        homepage/static/resume/katharina-lebenslauf.pdf \
        homepage/static/resume/katharina-anschreiben.pdf
git commit -m "# Wire CV + cover PDF download links + self-contained render-pdfs recipe"
```
...
The two PDFs are committed static assets. Deploy (Ansible) ships them and runs
`collectstatic` on the server like any other static file — **Chromium is never
installed on the server, never runs in a request**. Workflow when the CV/cover
content changes: `just render-pdfs` locally → commit the updated PDFs → deploy.

## Future (out of scope, noted)

- Auto-regenerate on CMS edit (content lives in the DB, so a true on-edit trigger is a save-signal hook — not wireable from this repo's file/deploy flow today); optional upgrade to on-demand rendering (design spec option A).
